...
export async function GET(request: NextRequest) {
  try {
    ...
    const aiCalls = calls.filter((c) => c.isVoiceAI).length;
    const manualCalls = totalCalls - aiCalls;
    ...
    const hourlyData = Object.entries(hourBuckets)
      .map(([hour, data]) => ({
        ...
        connectRate:
          data.total > 0
            ? Math.round((data.connected / data.total) * 100)
            : 0,
      }))
      .filter((h) => h.hour >= 8 && h.hour <= 20); // 8 AM to 8 PM

    const bestHour = hourlyData.reduce(
      (best, h) =>
        h.connected > best.connected ? h : best,
    ...
    for (const call of calls) {
      ...
      // Since calls are ordered desc, first encountered is most recent
